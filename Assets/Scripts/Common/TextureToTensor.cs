    ...
    public class TextureToTensor : System.IDisposable
    {
        ...
        public RenderTexture Resize(Texture texture, ResizeOptions options)
        {
            ...
            if (texture is WebCamTexture)
            {
                ...
                options.rotationDegree += webcamTex.videoRotationAngle + SROptions.Current.CameraDegreeOffset;
                // if (webcamTex.videoVerticallyMirrored)
                // {
                //     options.flipX = !options.flipX;
                // }
            }
            Matrix4x4 trs = GetVertTransform(options);
        ...
        public void ToTensor(RenderTexture texture, byte[,,] inputs, float offset, float scale)
        {
            // TODO: optimize this
            var pixels = FetchPixels(texture);
            int width = texture.width;
            for (int i = 0; i < pixels.Length; i++)
            {
                int y = i / width;
                int x = i % width;
                inputs[y, x, 0] = (byte)((pixels[i].r - offset) * scale);
                inputs[y, x, 1] = (byte)((pixels[i].g - offset) * scale);
                inputs[y, x, 2] = (byte)((pixels[i].b - offset) * scale);
            }
        }
        ...
        static Matrix4x4 GetVertTransform(ResizeOptions options)
        {
            Matrix4x4 trs = Matrix4x4.TRS(
                ...
                //     0),
                // Quaternion.identity,
                Quaternion.Euler(0, 0, options.rotationDegree),
                new Vector3(
                    options.flipX ? -options.scaleX : options.scaleX,
                    options.flipY ? -options.scaleY : options.scaleY,
                    1)
            );
            Matrix4x4 trs2 = Matrix4x4.TRS(
                new Vector3(
                    options.offsetX,
                    options.offsetY,
                    0),
            ...
            return PUSH_MATRIX * trs * trs2 * POP_MATRIX;
